...
>
        ...
        <div class="mt-[-20px] h-[calc(100%+20px)] rounded-2xl scrollable-y">
            ...
            {:else}
                <div class="mt-[74px]">
                    ...
                    {#if resident.active && resident.show && resident.link_telegram}
                    ...
                    {/if}
                    {#if resident.review}
                        <div class="flex justify-center ml-2 mt-1.5">
                            <button
                                class="text-base-100 rounded-xl min-h-[20px] flex items-center pl-2 pr-4 bg-secondary text-base-100"
                                on:click="{() => {
                                    reviewShow = !reviewShow;
                                }}"
                            >
                                <div class="w-5 h-5 shrink-0 mr-2 mb-0.5">
                                ...
                                <div
                                    class="text-xs leading-[20px] mt-[3px] mb-[3px] font-medium"
                                >Цифровой AI-портрет</div>
                            </button>
                        </div>
                    {/if}
                    ...
                    {:else}
                        ...
                        <div class="p-4 mx-3 mt-8 mb-5 bg-base-200 rounded-2xl flex items-start">
                            <svg class="w-6 h-6 text-error shrink-0 grow-0" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512"><path d="M256 8C119.043 8 8 119.083 8 256c0 136.997 111.043 248 248 248s248-111.003 248-248C504 119.083 392.957 8 256 8zm0 110c23.196 0 42 18.804 42 42s-18.804 42-42 42s-42-18.804-42-42s18.804-42 42-42zm56 254c0 6.627-5.373 12-12 12h-88c-6.627 0-12-5.373-12-12v-24c0-6.627 5.373-12 12-12h12v-64h-12c-6.627 0-12-5.373-12-12v-24c0-6.627 5.373-12 12-12h64c6.627 0 12 5.373 12 12v100h12c6.627 0 12 5.373 12 12v24z" fill="currentColor"></path></svg>
                            <span class="text-sm ml-3">Если Вы хотите связаться с этим человеком, обратитесь к комьюнити-менеджеру!</span>
                        </div>
    ...
    <ModalSelector
        bind:open="{reviewShow}"
    >
        <div class="w-full h-full p-2 overflow-y-auto">
            {#if resident && resident.review}
                {#each resident.review as r}
                    {#if r.answer}
                        <div class="bg-[rgba(0,181,251,0.5)] rounded-lg overflow-hidden p-1 mt-3 first:mt-0 border border-[rgb(0,181,251)]">
                            <div class="bg-[rgba(55,124,251,0.5)] py-2 pr-2 pl-2 rounded-lg overflow-hidden flex items-start justify-start border border-[rgb(55,124,251)]">
                                <svg class="shrink-0 grow-0 w-5 h-5 mr-2 text-base-100" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 24 24"><g fill="none"><path d="M2 10.5a7.5 7.5 0 1 1 4.411 6.836c-1.258.29-2.613.54-3.236.652a.996.996 0 0 1-1.153-1.17c.118-.61.38-1.918.681-3.143A7.474 7.474 0 0 1 2 10.5zm6.144-4.193c-.442.236-.79.603-1.053 1.09a.75.75 0 0 0 1.318.714c.147-.27.298-.404.442-.48c.15-.081.354-.13.649-.13c.224 0 .487.079.68.238c.171.14.32.364.32.761c0 .195-.075.285-.588.688c-.45.354-1.162.928-1.162 2.062a.75.75 0 0 0 1.5 0c0-.37.164-.549.588-.883l.085-.065C11.31 10.004 12 9.475 12 8.5c0-.848-.351-1.498-.868-1.922A2.614 2.614 0 0 0 9.5 6.001c-.455 0-.922.074-1.356.306zM10.5 14a1 1 0 1 0-2 0a1 1 0 0 0 2 0zm-1.1 5a7.474 7.474 0 0 0 5.1 2c1.1 0 2.146-.237 3.089-.664c1.26.29 2.621.54 3.248.65a.996.996 0 0 0 1.15-1.175a74.514 74.514 0 0 0-.69-3.136A7.503 7.503 0 0 0 16.954 6.41A8.5 8.5 0 0 1 9.4 19z" fill="currentColor"></path></g></svg>
                                <span class="text-xs font-medium">{r.question}</span>
                            </div>
                            <div class="py-2 pl-9 pr-2 text-xs font-semibold">{r.answer}</div>
                        </div>
                    {/if}
                {/each}
            {/if}
