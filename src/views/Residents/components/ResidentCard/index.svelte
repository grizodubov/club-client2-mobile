<script lang="ts">
    ...
    function info() {
        ...
        if (resident.position)
            temp += resident.position.toUpperCase();
        if (resident.company)
            temp += temp ? ' / ' + resident.company.toUpperCase() : resident.company.toUpperCase();
        return temp;
    }
</script>


<button on:click class="flex items-center justify-start px-4">
    <div class="w-[64px] h-[64px] shrink-0 grow-0">
        <Avatar user="{{
            id: resident.id,
            name: resident.name,
    ...
    <div class="ml-4 shrink-1 grow-1">
        <div class="flex items-center h-[28px]">
            <div class="text-left font-medium text-[17px] leading-[22px]">{nameNormalization(resident.name, 2)}</div>
            {#if resident.rating}
                <div class="w-5 h-5 ml-3 text-front">
            ...
            {#if resident.tagsLinked.length > 0 || resident.interestsLinked.length > 0}
                <div class="w-5 h-5 ml-3 text-success">
            ...
            {/if}
            {#if contact}
                <div class="w-5 h-5 ml-3 text-warning">
        ...
        <div class="text-left text-[10px] opacity-80">{info()}</div>
    </div>
</button>
