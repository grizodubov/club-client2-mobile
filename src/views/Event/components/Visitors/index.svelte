            ...
            {#if currentUser.status != confirmationUser.status && (currentUser.status == 'бронзовый' || (currentUser.status == 'серебряный' && confirmationUser.status == 'золотой'))}
                ...
                <div class="text-center text-sm px-3">
                    {#if currentUser.status == 'бронзовый'}
                        Для назначения встреч с членом клуба с серебряным или золотым статусом, обратитесь к своему комьюнити менеджеру
                    {:else}
                        Для назначения встреч с членом клуба с золотым статусом, обратитесь к своему комьюнити менеджеру
                    {/if}
                </div>
                <button
