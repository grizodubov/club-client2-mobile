<script lang="ts">
    ...
    import { eventPopupY } from '@/stores/scrolls';
...
<div class="w-full h-full flex flex-col px-2">
            ...
            <div class="flex items-center">
                <button
                    ...
                    on:click="{() => {
                        ...
                        dispatch('action', { action: 'online', online: filter.online });
                    }}"
                >
    ...
    <div class="w-full h-full shrink-1 grow-1 pb-4 scrollable-y" on:scroll="{setScrollBeacon}" bind:this="{scrollArea}">
        {#if data.speakers.length || data.participants.length}
            {#each [ ...data.speakers, ...data.participants, { id: 0 } ] as user (user.id)}
                {#if user.id !== 0}
                    {#if
